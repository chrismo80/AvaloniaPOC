...
public abstract class DeviceX(int id, string name, double value) : IDevice
{
	...
	public void Execute(string methodName, params object[] methodParameter)
	{
		...
		{
			case "DoStuff":
				this.CreateMessage(Name + " did stuff", MessageType.Information);
				break;
			case "DoThis":
				this.CreateMessage(Name + " did this", MessageType.Information);
				break;
			case "DoThat":
				this.CreateMessage(Name + " did that", MessageType.Information);
				break;
		}
	}
